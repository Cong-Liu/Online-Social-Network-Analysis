...
def volume(nodes, graph):
    ...
    return s-nx.number_of_edges(graph.subgraph(nodes))
    pass
...
def score_max_depths(graph, max_depths):
    ...
    for md in max_depths:
        #         print()
        #         print ('graph comp: ',len(list(nx.connected_component_subgraphs(graph))))
        subgraphs = partition_girvan_newman(g, md)
        #         print ('max depths: ',md,'nums subgraphs: ',len(subgraphs))
        score.append((md, norm_cut(subgraphs[0], subgraphs[1], g)))
        g = graph.copy()
    # sub = partition_girvan_newman(g, max_depths)
    # return [(max_depths, norm_cut(sub[0], sub[1], g))]
    return score
    pass
